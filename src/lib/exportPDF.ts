...
function kpiBox(doc: jsPDF, x: number, y: number, w: number, h: number, label: string, value: string, color: number[]) {
  ...
  // Text color: dark if light background, else white
...
function barChart(doc: jsPDF, x: number, y: number, w: number, h: number, data: {label:string; value:number; color:number[]}[], title: string) {
  ...
  const barW = Math.min(20, (w - 10) / data.length - 4);
  const spacing = (w - 10) / data.length;
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    doc.setFontSize(6);
    doc.setFont('helvetica', 'normal');
    doc.text(formatCFA((maxVal * i) / 4).replace(' FCFA', 'k').replace(/\s/g,''), x - 1, ly + 1, { align: 'right' });
  }
  ...
  data.forEach((d, i) => {
    ...
    doc.setFontSize(6);
    doc.setFont('helvetica', 'normal');
    doc.text(d.label, bx + barW / 2, y + h + 4, { align: 'center' });
  });
}
...
export async function exportStatsPDF(data: any, company: string, logoUrl?: string|null, primaryColor?: string|null) {
  ...
  kpiBox(doc, 10, y, kpiW, kpiH, 'Biens loués', `${data.rented}/${data.props} (${data.occupancy}%)`, [219, 234, 254]);
  kpiBox(doc, 10+kpiW+gap, y, kpiW, kpiH, 'Locataires actifs', `${data.activeTenants}/${data.tenants}`, [220, 252, 231]);
  kpiBox(doc, 10+(kpiW+gap)*2, y, kpiW, kpiH, 'Revenus période', formatCFA(data.revenue), [233, 213, 255]);
  kpiBox(doc, 10+(kpiW+gap)*3, y, kpiW, kpiH, 'Bénéfice net', formatCFA(data.net), [233, 213, 255]);
  y += kpiH + gap;

  kpiBox(doc, 10, y, kpiW, kpiH, `Commissions (${data.commissionRate}%)`, formatCFA(data.commissions), [219, 234, 254]);
  kpiBox(doc, 10+kpiW+gap, y, kpiW, kpiH, 'Total dépenses', formatCFA(data.expenses), [254, 226, 226]);
  kpiBox(doc, 10+(kpiW+gap)*2, y, kpiW, kpiH, 'Taux recouvrement', `${data.collectionRate}%`, [220, 252, 231]);
  kpiBox(doc, 10+(kpiW+gap)*3, y, kpiW, kpiH, "Taux d'impayés", `${data.impayeRate}%`, data.impayeRate === 0 ? [220, 252, 231] : [254, 226, 226]);
  y += kpiH + 10;
  ...
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  doc.text(`Sur ${data.period} mois : ${formatCFA(data.revenue)} de revenus · ${formatCFA(data.commissions)} de commissions · ${formatCFA(data.expenses)} de dépenses · Bénéfice net : ${formatCFA(data.net)}`, 105, y + 9, { align: 'center', maxWidth: 180 });
  y += 20;

  // Bar chart
  y = sectionTitle(doc, y, 'Évolution mensuelle');
  const chartData = data.chart.map((c: any) => ([
    { label: c.month, value: c.revenue, color: [34, 197, 94] },
    { label: c.month, value: c.commissions, color: [59, 130, 246] },
    { label: c.month, value: c.expenses, color: [249, 115, 22] },
    { label: c.month, value: c.net, color: [168, 85, 247] },
  ])).flat();

  // Simplified monthly table instead of chart
  ...
  data.chart.forEach((c: any, i: number) => {
    ...
    doc.text(formatCFA(c.revenue), cols[1] + 2, y + 5);
    doc.setTextColor(59, 130, 246);
    doc.text(formatCFA(c.commissions), cols[2] + 2, y + 5);
    doc.setTextColor(239, 68, 68);
    doc.text(formatCFA(c.expenses), cols[3] + 2, y + 5);
    doc.setTextColor(168, 85, 247);
    doc.text(formatCFA(c.net), cols[4] + 2, y + 5);
    y += 7;
  });
  y += 8;
  ...
  doc.setFontSize(18);
  doc.setFont('helvetica', 'bold');
  doc.text(`${data.occupancy}%`, 55, y + 13, { align: 'center' });
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  ...
  doc.setFontSize(18);
  doc.setFont('helvetica', 'bold');
  doc.text(String(data.rented), 126, y + 13, { align: 'center' });
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  ...
  doc.setFontSize(18);
  doc.setFont('helvetica', 'bold');
  doc.text(String(data.available), 176, y + 13, { align: 'center' });
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  ...
  if (data.expCats && data.expCats.length > 0) {
    ...
    const totalExp = data.expCats.reduce((s: number, e: any) => s + e.value, 0);
    data.expCats.forEach((e: any) => {
      ...
      doc.setFontSize(7);
      doc.setFont('helvetica', 'bold');
      doc.text(`${e.name} — ${formatCFA(e.value)} (${pct}%)`, 14, y + 5);
      y += 9;
    });
  }
...
export async function exportReportPDF(data: any, company: string, logoUrl?: string|null, primaryColor?: string|null) {
  ...
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  const summary = `Ce mois-ci, la plateforme a généré ${formatCFA(data.currentMonthRevenue)} de revenus avec un taux de recouvrement de ${data.collectionRate}%, pour un bénéfice net de ${formatCFA(data.currentMonthNet)}. Parc : ${data.totalProperties} bien(s) · ${data.activeTenants} locataire(s) actif(s).`;
  doc.text(summary, 15, y + 7, { maxWidth: 180 });
  y += 22;

  // KPIs
  const kpiW = 43; const kpiH = 18; const gap = 3;
  kpiBox(doc, 10, y, kpiW, kpiH, 'Revenus du mois', formatCFA(data.currentMonthRevenue), [220, 252, 231]);
  kpiBox(doc, 10+kpiW+gap, y, kpiW, kpiH, 'Dépenses du mois', formatCFA(data.currentMonthExpenses), [254, 226, 226]);
  kpiBox(doc, 10+(kpiW+gap)*2, y, kpiW, kpiH, 'Bénéfice net', formatCFA(data.currentMonthNet), [233, 213, 255]);
  kpiBox(doc, 10+(kpiW+gap)*3, y, kpiW, kpiH, 'Taux recouvrement', `${data.collectionRate}%`, [220, 252, 231]);
  y += kpiH + 8;
  ...
  [
    { label: 'Loyers collectés', value: data.collectedRents, var: `${data.revenueGrowth >= 0 ? '+' : ''}${data.revenueGrowth}%`, status: '✓ Collectés', color: [22, 163, 74] },
    { label: `Commissions (${data.commissionRate}%)`, value: data.totalCommissions, var: 'Auto', status: 'Générées', color: [59, 130, 246] },
    { label: 'En attente', value: data.pendingRents, var: '—', status: 'À collecter', color: [161, 98, 7] },
    { label: 'En retard', value: data.overdueRents, var: '—', status: 'Impayés', color: [220, 38, 38] },
  ].forEach((row, i) => {
  ...
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  doc.text(`${formatCFA(data.currentMonthRevenue)} (revenus)  −  ${formatCFA(data.currentMonthRevenue*(data.commissionRate/100))} (commissions)  −  ${formatCFA(data.currentMonthExpenses)} (dépenses)  =`, 105, y+8, { align: 'center' });
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text(formatCFA(data.currentMonthNet), 105, y+17, { align: 'center' });
  y += 28;
  ...
  doc.setFontSize(11);
  doc.setFont('helvetica', 'bold');
  doc.text(formatCFA(data.totalBailleurExp), 14, y+12);
  ...
  doc.setFontSize(11);
  doc.setFont('helvetica', 'bold');
  doc.text(formatCFA(data.totalEntrepriseExp), 109, y+12);
  y += 20;

  if (data.expensesByCategory && data.expensesByCategory.length > 0) {
    const totalExp = data.expensesByCategory.reduce((s: number, e: any) => s + e.amount, 0);
    data.expensesByCategory.slice(0, 5).forEach((e: any) => {
      const pct = totalExp > 0 ? Math.round((e.amount / totalExp) * 100) : 0;
      doc.setFillColor(241, 245, 249);
      doc.roundedRect(10, y, 190, 7, 2, 2, 'F');
      doc.setFillColor(59, 130, 246, 0.6);
      doc.roundedRect(10, y, Math.max(2, (pct / 100) * 190), 7, 2, 2, 'F');
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(7);
      doc.setFont('helvetica', 'bold');
      doc.text(`${e.name}  ${formatCFA(e.amount)}  (${pct}%)`, 14, y+5);
      y += 9;
    });
  }
  y += 5;
  ...
  kpiBox(doc, 10, y, 55, 16, 'Biens loués', `${data.rentedProps} / ${data.totalProperties}`, [219, 234, 254]);
  kpiBox(doc, 70, y, 55, 16, 'Disponibles', String(data.availableProps), [220, 252, 231]);
  kpiBox(doc, 130, y, 70, 16, "Taux d'occupation", `${data.occupancyRate}%`, [233, 213, 255]);
  y += 24;

  // ── SUIVI LOCATAIRES ──
  y = sectionTitle(doc, y, 'Suivi des locataires');
  kpiBox(doc, 10, y, 55, 16, 'Locataires actifs', String(data.activeTenants), [219, 234, 254]);
  kpiBox(doc, 70, y, 55, 16, 'Paiements à jour', String(data.paidTenants), [220, 252, 231]);
  kpiBox(doc, 130, y, 70, 16, 'En retard', String(data.lateTenants), data.lateTenants > 0 ? [254, 226, 226] : [220, 252, 231]);
  y += 22;
  ...
  if (data.topPayers && data.topPayers.length > 0) {
    ...
    data.topPayers.forEach((t: any, i: number) => {
      ...
      doc.setTextColor(22, 163, 74);
      doc.setFont('helvetica', 'bold');
      doc.text(formatCFA(t.amount), 196, y+5, { align: 'right' });
      y += 7;
    });
    y += 5;
  }
  ...
  kpiBox(doc, 10, y, 55, 16, 'Tickets ouverts', String(data.openTickets), data.openTickets > 0 ? [254, 243, 199] : [220, 252, 231]);
  kpiBox(doc, 70, y, 55, 16, 'Résolus', String(data.resolvedTickets), [220, 252, 231]);
  kpiBox(doc, 130, y, 70, 16, 'Total signalements', String(data.totalTickets), [219, 234, 254]);
  y += 22;
  ...
  if (data.ticketsByCategory && data.ticketsByCategory.length > 0) {
    data.ticketsByCategory.forEach((t: any, i: number) => {
      ...
      doc.setFont('helvetica', 'bold');
      doc.text(String(t.count), 196, y+5, { align: 'right' });
      y += 7;
    });
    y += 5;
  }
  ...
  data.monthlyChart.forEach((c: any, i: number) => {
    ...
    doc.text(formatCFA(c.revenue), c2[1]+2, y+5);
    doc.setTextColor(59, 130, 246);
    doc.text(formatCFA(c.commissions), c2[2]+2, y+5);
    doc.setTextColor(239, 68, 68);
    doc.text(formatCFA(c.expenses), c2[3]+2, y+5);
    doc.setTextColor(168, 85, 247);
    doc.text(formatCFA(c.net), c2[4]+2, y+5);
    y += 7;
  });
  y += 8;
  ...
  kpiBox(doc, 10, y, 55, 18, 'Revenus estimés', formatCFA(data.forecastRevenue), [204, 251, 241]);
  kpiBox(doc, 70, y, 55, 18, 'Dépenses prévues', formatCFA(data.forecastExpenses), [254, 243, 199]);
  kpiBox(doc, 130, y, 70, 18, 'Bénéfice net prévu', formatCFA(Math.max(0, data.forecastRevenue - data.forecastRevenue*(data.commissionRate/100) - data.forecastExpenses)), [233, 213, 255]);
