...
export default function EditCompanyPage() {
  ...
  const [form, setForm] = useState({
    name: '', email: '', phone: '', address: '', plan: 'starter', modules: [] as string[], is_active: true,
  });
  ...
  const handleSave = async () => {
    ...
    const { error } = await createClient()
      ...
      .update({ name: form.name, email: form.email || null, phone: form.phone || null,
                address: form.address || null, plan: form.plan, modules: form.modules,
                is_active: form.is_active } as never)
      .eq('id', id);
    setSaving(false);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className={cardCls + ' p-6 space-y-4'}>
        ...
        <div>
          <label className={labelCls}>Modules activés</label>
          <div className="flex gap-3 mt-1">
            {MODULES.map(m => (
              <button key={m.id} onClick={() => toggleModule(m.id)}
                className={'px-4 py-2 rounded-xl border-2 text-sm font-medium transition-all ' +
                  (form.modules.includes(m.id) ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-primary/40')}>
                {m.label}
              </button>
            ))}
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setForm(f => ({ ...f, is_active: !f.is_active }))}
            className={'px-4 py-2 rounded-xl text-sm font-medium border-2 transition-all ' +
              (form.is_active ? 'border-green-500 bg-green-50 text-green-700' : 'border-red-300 bg-red-50 text-red-600')}>
