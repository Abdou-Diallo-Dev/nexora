...
export default function REDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const allPaid    = PAY.filter((p: any) => p.status === 'paid' || p.status === 'partial');
      ...
      const totalComm     = calcCommTTC(totalLoyers, commRate);
      ...
      const chart = Array.from({ length: 6 }, (_, i) => {
        ...
        return {
          ...
          revenue: allPaid.filter((p: any) => `${p.period_month}/${p.period_year}` === mo).reduce((s: number, p: any) => s + Number(p.amount), 0),
        };
      });
  ...
  return (
    <div className="space-y-6">
      ...
      {sections.showRevenue && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { title: 'Commissions TTC',      value: formatCurrency(data.totalCommissions), color: 'text-blue-700',   bg: 'bg-blue-50 dark:bg-blue-900/20 border-blue-100 dark:border-blue-800' },
                    ...
                    <Link href={'/real-estate/properties/' + p.id}>
                      <div className="relative w-full h-64 md:h-80 bg-slate-100 dark:bg-slate-700">
                        {imgs.length > 0
                          ...
                          : <div className="w-full h-full flex items-center justify-center"><Home size={48} className="text-slate-300"/></div>
                        }
