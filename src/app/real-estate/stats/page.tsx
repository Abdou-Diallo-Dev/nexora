...
export default function StatsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const prevPaid = (PAY as any[]).filter((p: any) =>
        moKey(new Date(p.period_year, p.period_month - 1, 1)) === prevMo &&
      ...
      const chart = Array.from({ length: months }, (_, i) => {
        ...
        const rev = (PAY as any[])
          .filter((p: any) => (p.status === 'paid' || p.status === 'partial') && moKey(new Date(p.period_year, p.period_month - 1, 1)) === moK)
          .reduce((s: number, p: any) => s + p.amount, 0);
        const exp2 = (EXP as any[])
          .filter((e: any) => moKey(new Date(e.date)) === moK)
          .reduce((s: number, e: any) => s + e.amount, 0);
        const comm = calcCommTTC(rev, commRate); // ✅ TTC
      ...
      const expCats = Object.entries(cats)
        .sort((a, b) => b[1] - a[1])
        .map(([name, value], i) => ({ name, value, color: COLORS[i % COLORS.length] }));

      setD({
        props: P.length, rented, available,
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { label: `Commissions TTC (HT ${d.commissionRate}%+TVA 18%)`, value: formatCurrency(d.commissions), sub: 'Générées auto. (TTC)', color: 'text-blue-700', bg: 'bg-blue-50 dark:bg-blue-900/20 border-blue-100', icon: <Percent size={15}/> },
