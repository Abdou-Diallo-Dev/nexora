...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    // Si admin/super_admin : voit toutes les conversations
    // Si autres rôles : voit seulement ses propres échanges avec ce locataire
    if (user?.role !== 'admin' && user?.role !== 'super_admin') {
      msgQuery = msgQuery.or(`sender_id.eq.${user?.id},sender_role.eq.tenant`);
    }

    msgQuery.order('created_at', { ascending: true })
      .then(({ data }) => {
