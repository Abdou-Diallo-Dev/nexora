...
import { FileText, FileCheck, Download, Receipt, ScrollText, Loader2 } from 'lucide-react';
import { createClient } from '@/lib/supabase/client';
import { useAuthStore } from '@/lib/store';
import { PageHeader, LoadingSpinner, EmptyState, Badge, cardCls, btnPrimary } from '@/components/ui';
...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    ...
    createClient()
      ...
      .select(`
        ...
        deposit_amount, payment_day, status,
        tenants(first_name, last_name, email, phone),
        properties(name, address, city, type)
      `)
  ...
  // Génère la quittance pour le mois en cours
  ...
  // Génère le contrat de bail complet
  ...
  const isGenerating = (id: string, type: 'receipt' | 'contract') =>
  ...
  return (
    <div>
      <PageHeader
        title="Documents & PDF"
        subtitle="Générez vos quittances et contrats officiels"
      />
      ...
      <div className="mb-5 flex items-start gap-3 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
        ...
        <div>
          <p className="text-sm font-medium text-blue-900 dark:text-blue-200">
            Période en cours : <strong>{currentPeriod}</strong>
          </p>
          <p className="text-xs text-blue-700 dark:text-blue-300 mt-0.5">
            Les quittances seront générées pour le mois en cours. Les contrats reprennent les données complètes du bail.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Créez des baux pour générer des documents PDF"
        />
      ) : (
        <div className={cardCls}>
          ...
          <div className="divide-y divide-border">
            {leases.map(l => {
              const tenantName = l.tenants
                ? `${l.tenants.first_name} ${l.tenants.last_name}`
                : '—';
              const propName = l.properties?.name || '—';
              const propCity = l.properties?.city || '';

              return (
                <div
                  key={l.id}
                  className="grid grid-cols-1 md:grid-cols-[1fr_140px_160px_160px] gap-4 px-5 py-4 items-center hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors"
                >
                  {/* Info */}
                  ...
                  <div>
                    ...
                    {l.charges_amount > 0 && (
                      <p className="text-xs text-muted-foreground">
                        +{formatCurrency(l.charges_amount)} charges
                      </p>
                    )}
                    <Badge
                  ...
                  <div className="flex justify-center">
                    <button
                      ...
                      disabled={!!generating}
                      className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl text-xs font-semibold transition-colors shadow-sm w-full justify-center"
                    >
                      {isGenerating(l.id, 'receipt')
                  ...
                  <div className="flex justify-center">
                    <button
                      onClick={() => handleContract(l)}
                      disabled={!!generating}
                      className="flex items-center gap-2 px-4 py-2.5 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white rounded-xl text-xs font-semibold transition-colors shadow-sm w-full justify-center"
                    >
                      {isGenerating(l.id, 'contract')
                        ? <Loader2 size={13} className="animate-spin" />
                        : <ScrollText size={13} />
                      }
                      Contrat PDF
          ...
          <div className="px-5 py-3 border-t border-border bg-slate-50 dark:bg-slate-700/20">
            <p className="text-xs text-muted-foreground">
              {leases.length} bail(s) · Les fichiers sont téléchargés directement en format PDF
            </p>
          </div>
        </div>
      )}
