...
export default function ReportsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      sb.from('rent_payments').select('id,amount,paid_amount,status,period_month,period_year,paid_date,tenant_id,lease_id').eq('company_id', cid).limit(1000),
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        <h2 className="text-base font-bold text-foreground mb-3 flex items-center gap-2">
          <span className="w-1 h-5 bg-green-500 rounded-full inline-block"/>Détail des revenus
        </h2>
      ...
      <div>
        <h2 className="text-base font-bold text-foreground mb-3 flex items-center gap-2">
          <span className="w-1 h-5 bg-red-500 rounded-full inline-block"/>Détail des dépenses
        </h2>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className={cardCls + ' p-5'}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={data.expensesByCategory} cx="50%" cy="50%" outerRadius={80} dataKey="amount"
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <div className="flex items-center gap-3 flex-wrap text-sm font-mono">
                ...
                {/* ✅ Affiche la commission TTC réelle */}
                <span className="text-blue-700 font-semibold">{formatCurrency(data.totalCommissions)}</span>
                <span className="text-muted-foreground">−</span>
      ...
      <div>
        <h2 className="text-base font-bold text-foreground mb-3 flex items-center gap-2">
          <span className="w-1 h-5 bg-blue-500 rounded-full inline-block"/>Performance des biens
        </h2>
      ...
      <div>
        <h2 className="text-base font-bold text-foreground mb-3 flex items-center gap-2">
          <span className="w-1 h-5 bg-green-500 rounded-full inline-block"/>Suivi des locataires
        </h2>
        <div className="grid grid-cols-3 gap-4 mb-4">
      ...
      <div>
        <h2 className="text-base font-bold text-foreground mb-3 flex items-center gap-2">
          <span className="w-1 h-5 bg-orange-500 rounded-full inline-block"/>Signalements & maintenance
        </h2>
        <div className="grid grid-cols-3 gap-4 mb-4">
      ...
      <div>
        <h2 className="text-base font-bold text-foreground mb-3 flex items-center gap-2">
          <span className="w-1 h-5 bg-indigo-500 rounded-full inline-block"/>Indicateurs clés (KPI)
        </h2>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Taux de recouvrement', pct: data.collectionRate, color: data.collectionRate >= 80 ? '#22c55e' : '#f59e0b' },
            { label: "Taux d'occupation",    pct: data.occupancyRate,  color: '#3b82f6' },
          ].map((k, i) => (
            <div key={i} className={cardCls + ' p-4 text-center'}>
              <p className="text-xs text-muted-foreground uppercase font-semibold mb-2">{k.label}</p>
              <div className="relative w-16 h-16 mx-auto mb-2">
                <svg viewBox="0 0 36 36" className="w-16 h-16 -rotate-90">
                  <circle cx="18" cy="18" r="15.9" fill="none" stroke="#e2e8f0" strokeWidth="3"/>
                  <circle cx="18" cy="18" r="15.9" fill="none" stroke={k.color} strokeWidth="3"
                    strokeDasharray={`${k.pct} ${100 - k.pct}`} strokeLinecap="round"/>
                </svg>
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-xs font-bold text-foreground">{k.pct}%</span>
                </div>
              </div>
            </div>
          ))}
