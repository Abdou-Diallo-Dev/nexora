...
export default function EditChauffeurPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('drivers').select('*').eq('id', id).eq('company_id', company.id).single()
      .then(({ data: d }) => {
        if (!d) { router.push('/logistics/drivers'); return; }
        setData(d);
        setLoading(false);
      });
  }, [id, company?.id]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const supabase = createClient();
    const { error } = await supabase.from('drivers').update(data).eq('id', id as string);
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-foreground">Modifier — Chauffeur</h1>
      </div>
