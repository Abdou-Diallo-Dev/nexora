...
export default function DeliveriesPage() {
  ...
  const load = () => {
    ...
    q.then(
      ({ data, count }) => { setItems((data||[]) as unknown as Delivery[]); setTotal(count||0); setLoading(false); },
      (err: any) => { console.error('Erreur livraisons:', err); toast.error('Erreur: ' + (err?.message || 'requête échouée')); setLoading(false); }
    );
  };
