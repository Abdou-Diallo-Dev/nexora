...
import {
  ...
  Shield, Crown, Lock, LayoutDashboard, Zap,
} from 'lucide-react';
...
const RE_GROUP_MAP: { label: string; keys: string[] }[] = [
...
function NavGroupComp({ group, collapsed, pathname }: { group: NavGroup; collapsed: boolean; pathname: string }) {
...
export default function Sidebar() {
  ...
  // Build filtered RE nav based on role
  ...
  return (
    <motion.aside animate={{width: collapsed ? 60 : 236}} transition={{duration:0.2,ease:'easeInOut'}}
      className="h-screen flex-shrink-0 bg-white dark:bg-slate-800 border-r border-border flex flex-col overflow-hidden z-40">
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-2">
        {nav ? (
          nav.map(g => <NavGroupComp key={g.label} group={g} collapsed={collapsed} pathname={pathname}/>)
        ) : (
      ...
      {!collapsed && nav && (
        <div className="px-3 py-3 border-t border-border space-y-1">
          {!isRE && hasRE && (
            <Link href="/real-estate" className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs text-muted-foreground hover:text-blue-600 hover:bg-blue-50 transition-colors">
              <Building2 size={13}/> Nexora Immo
            </Link>
          )}
          {!isLog && hasLog && (
            <Link href="/logistics" className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs text-muted-foreground hover:text-green-600 hover:bg-green-50 transition-colors">
              <Truck size={13}/> Nexora Logistique
            </Link>
          )}
          {role === 'super_admin' && !isSA && (
            <Link href="/super-admin/dashboard" className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs text-red-600 hover:bg-red-50 transition-colors font-medium">
