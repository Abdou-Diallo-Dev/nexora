...
-- This migration enables RLS on tables missing it and adds proper access policies
...
-- 2. Create RLS policies for supplier_order_items
DROP POLICY IF EXISTS "supplier_order_items_company_select" ON public.supplier_order_items;
CREATE POLICY "supplier_order_items_company_select" ON public.supplier_order_items
  FOR SELECT USING (
    order_id IN (
      SELECT id FROM supplier_orders WHERE company_id = auth.jwt() ->> 'company_id'
    )
  );

DROP POLICY IF EXISTS "supplier_order_items_company_insert" ON public.supplier_order_items
...
DROP POLICY IF EXISTS "supplier_order_items_company_update" ON public.supplier_order_items
  FOR UPDATE USING (
    order_id IN (
      SELECT id FROM supplier_orders WHERE company_id = auth.jwt() ->> 'company_id'
    )
...
DROP POLICY IF EXISTS "supplier_order_items_company_delete" ON public.supplier_order_items
  FOR DELETE USING (
    order_id IN (
      SELECT id FROM supplier_orders WHERE company_id = auth.jwt() ->> 'company_id'
    )
...
-- 3. Create RLS policies for logistics_invoice_items
DROP POLICY IF EXISTS "logistics_invoice_items_company_select" ON public.logistics_invoice_items
  FOR SELECT USING (
    invoice_id IN (
      SELECT id FROM logistics_invoices WHERE company_id = auth.jwt() ->> 'company_id'
    )
  );

DROP POLICY IF EXISTS "logistics_invoice_items_company_insert" ON public.logistics_invoice_items
  FOR INSERT WITH CHECK (
    invoice_id IN (
      SELECT id FROM logistics_invoices WHERE company_id = auth.jwt() ->> 'company_id'
    )
...
DROP POLICY IF EXISTS "logistics_invoice_items_company_update" ON public.logistics_invoice_items
  FOR UPDATE USING (
    invoice_id IN (
      SELECT id FROM logistics_invoices WHERE company_id = auth.jwt() ->> 'company_id'
    )
...
DROP POLICY IF EXISTS "logistics_invoice_items_company_delete" ON public.logistics_invoice_items
  FOR DELETE USING (
    invoice_id IN (
      SELECT id FROM logistics_invoices WHERE company_id = auth.jwt() ->> 'company_id'
    )
...
-- These views should use RLS-enforced base tables, not override permissions

DROP VIEW IF EXISTS public.role_templates_view CASCADE;
CREATE VIEW public.role_templates_view AS
...
CREATE VIEW public.maintenance_stock_summary AS
SELECT 
...
CREATE VIEW public.logistics_finance_summary AS
SELECT 
...
CREATE VIEW public.company_directory AS
SELECT 
...
-- Ensure proper RLS on source tables
